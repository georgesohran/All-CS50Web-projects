...
void edges(int height, int width, RGBTRIPLE image[height][width])
{
    ...
    for (int i = 0; i < height; i++)
    {
        for (int j = 0; j < width; j++)
        {
            ...
            temp[i][j].rgbtRed = sqrt((gx_red * gx_red) + (gy_red * gy_red));
            temp[i][j].rgbtBlue = sqrt((gx_blue * gx_blue) + (gy_blue * gy_blue));
            temp[i][j].rgbtGreen = sqrt((gx_green * gx_green) + (gy_green * gy_green));
        }
    }
